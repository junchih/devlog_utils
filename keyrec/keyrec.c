...
CGEventRef CGEventCallback(
	...
	struct CGEventContext *ctx = (struct CGEventContext *)(refcon);
	...
	if (type == kCGEventFlagsChanged) {
		...
		case 55: // [left-cmd]
			...
			preDown = ctx->lastFlags & kCGEventFlagMaskCommand;
			break;
		case 56: // [left-shift]
		case 60: // [right-shift]
			down = flags & kCGEventFlagMaskShift;
			preDown = ctx->lastFlags & kCGEventFlagMaskShift;
			break;
		case 58: // [left-option]
		case 61: // [right-option]
			down = flags & kCGEventFlagMaskAlternate;
			preDown = ctx->lastFlags & kCGEventFlagMaskAlternate;
			break;
		case 59: // [left-ctrl]
		case 62: // [right-ctrl]
			down = flags & kCGEventFlagMaskControl;
			preDown = ctx->lastFlags & kCGEventFlagMaskControl;
			break;
		case 57: // [caps]
			down = flags & kCGEventFlagMaskAlphaShift;
			preDown = ctx->lastFlags & kCGEventFlagMaskAlphaShift;
			break;
		default:
			break;
		}
		...
		preDown = ctx->keyStats[keyCode];
		ctx->keyStats[keyCode] = down;
	...
	time_t now;
	time(&now);
	double utc = difftime(now, ctx->utcStart);
    fprintf(ctx->logfile, "%.f,%s,%s\n", utc, down?"D":"U", convertKeyCode(keyCode));
    fflush(ctx->logfile);
    return event;
}
